...
def check_approval(ctx: Context, node_input: dict):
    """Determine if the expense requires human approval.

    Stores the expense in context state for later steps.
    Routes to "requires_approval" if amount > 100, otherwise auto‑approves.
    """
    ctx.state["expense"] = node_input
    amount = node_input.get("amount", 0.0)
    if amount > 100.0:
        return Event(output=node_input, actions=EventActions(route="requires_approval"))
    else:
        decision = {"approved": True, "reason": "Automatic approval for expenses under $100."}
        return Event(output=decision, actions=EventActions(route="auto_approved"))
...
async def request_approval(ctx: Context, node_input: dict):
    ...
    manager_reply = ctx.resume_inputs["manager_approval"].strip().lower()
    approved = manager_reply in ("yes", "y", "approve")
    decision = {
        "approved": approved,
...
root_agent = Workflow(
    ...
    #(security_check, parse_expense, "clean"),
    #(security_check, request_approval, "security_review"),
     (security_check, parse_expense),
    (parse_expense, check_approval),
    (check_approval, request_approval),
    (check_approval, finalize),

    #(check_approval, request_approval, "requires_approval"),
    #(check_approval, finalize, "auto_approved"),
